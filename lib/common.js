...
Seq.prototype.derive = function(element, metadata) {
    if(!this.left.isNullable()) {
        ...
        return Or(Seq(this.left.derive(element, metadata), this.right), Seq(RedMany(this.left.delta(), func), this.right.derive(element, metadata)));
    }
};
...
Not.prototype.derive = function(element, metadata) {
    return RedMany(Not(this.expr.derive(element, metadata)), function(x) { return [[element].concat(x)]; });
...
function RedMany(expr, func) {
    if(this.constructor === RedMany) {
        this.expr = expr;
        this.func = func;
        
        return this.simplify();
    } else {
        return new RedMany(expr, func);
    }
}

util.inherits(RedMany, Red);

RedMany.prototype.derive = function(element, metadata) {
    return RedMany(this.expr.derive(element, metadata), function(){
        return this.func.apply(null, Array.prototype.slice.apply(arguments).concat([metadata]));
    }.bind(this));
};

RedMany.prototype.parseNull = function() {
    var result = [];
