...
Seq.prototype.derive = function(element) {
    if(this.left.delta().equals(Null())) {
        return Seq(this.left.derive(element), this.right);
    } else {
        return Or(Seq(this.left.derive(element), this.right), Seq(Red(Empty(), this.left.parseNull.bind(this.left)), this.right.derive(element)));
    }
};
...
console.log("tree", x.concat(y));
            result = result.concat(x.concat(y));
        }.bind(this));
    }.bind(this));
...
Red.prototype.toString = function() {
    // debug
    return 'Red(' + this.expr.toString() + ', ' + this.func.toString().replace(/\n/g, ' ') + ')';
