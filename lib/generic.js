...
Ref.prototype.derive = function(element, metadata) {
    var key = String(element) + (metadata === undefined ? '' : ':' + metadata.line + ':' + metadata.char);
    ...
    for(var i = 0; i < str.length; i++) {
        ...
        if(report) {
            if(str[i] instanceof Node) {
                ...
                line = lines.length;
                char = lines.pop().length + 1;
                
                metadata = {char: char, line: line};
            }
        }
        ...
        if(report && expr.equals(Void())) {
            if(str[i] instanceof Node) {
                throw new Error("Parse error: unexpected token " + str[i].toString() + " at " + metadata.line + ':' + metadata.char);
            } else {
                throw new Error("Parse error: unexpected character " + JSON.stringify(str[i]) + " at " + metadata.line + ":" + metadata.char);
            }
        }
    }
